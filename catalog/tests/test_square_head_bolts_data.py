...
DATA = Path("catalog/dimensions/square_head_bolts.json")
_FORBIDDEN = ("reyher", "stalmut")

_BASES = ("din478",)
_ALIASES = {"din478p": "din478"}
_DIN_CODE = {"din478": "478", "din478p": "478"}
...
def test_family_and_hardware_type():
    ...
    for sid, entry in entries.items():
        ...
        if "alias_of" in entry:
            continue
        assert entry["family"] == "square_head_bolt", f"{sid}: unexpected family {entry['family']}"
...
def test_din478_base_keeps_the_collared_square_form():
    # The DIN 478 form invariants: the collar washer face overhangs the sharp square
    # corners, and the total head height k (a DIN letter that INCLUDES the collar) leaves
    # a square band above the collar thickness c.
    shape = json.loads(DATA.read_text())["din478"]["shape"]
    assert shape["dc"] > shape["s"] * math.sqrt(2.0)
    assert shape["k"] > shape["c"]
    assert shape["d_shank"] < shape["dc"]


def test_aliases_resolve_to_a_real_base_without_chaining():
    entries = json.loads(DATA.read_text())
    for alias_id, base_id in _ALIASES.items():
        assert alias_id in entries, f"{alias_id} missing from square_head_bolts.json"
