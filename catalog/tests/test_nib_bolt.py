...
# and DIN 607 cup head (dk/k/g/i from Fuller + fasten.it). Both standards read i as the RADIAL
# nose height. The DIN 604 wedge is fully determined by dk/k/i, with two corners on existing
# silhouette vertices (per the figure and the legacy raster): top corner = the head-rim corner
# (12.325, 7), back face landing on the cone-shank junction (6, 0), nose corner at
# (6 + 5.7, 5.7*tan15) = (11.7, 1.53). The DIN 607 nib is a wedge on the shank — reach nib_l at
# z=0, underside sloping to the shank at -nib_d (30-deg slope).
D_SHANK = 12.0
LENGTH = 50.0
DK = 24.65
...
def _csk(**over):
    cfg = dict(d_shank=D_SHANK, length=LENGTH, dk=DK, k=K_CSK, head_style="countersunk",
               nib_w=NIB_W, nib_d=NIB_D_CSK, tip_chamfer=TIP_CHAMFER)
    cfg.update(over)
    return nib_bolt(**cfg)
...
def test_countersunk_nib_wedge_alongside_the_cone_on_plus_x_only():
    # outer edge runs (12.325, 7) -> (11.7, 1.53), back face (11.7, 1.53) -> (6, 0): probes
    # beyond the cone (radius 6 + 0.9036*z) hit wedge material only above those boundaries.
    part = _csk()
    assert _solid_at(part, 10.0, 0.0, 1.8)                  # wedge material beyond the cone
    assert _solid_at(part, 8.5, 0.0, 0.9)                   # inside the back-face region
    assert not _solid_at(part, -10.0, 0.0, 1.8)             # one-sided: nothing on -X
    assert not _solid_at(part, DK / 2.0 + 0.7, 0.0, K_CSK - 0.5)    # nothing past the rim
    assert not _solid_at(part, 10.0, 0.0, 0.5)              # void under the 15-deg back face
    assert not _solid_at(part, 8.0, 0.0, -0.5)              # the wedge stays above z=0
...
def test_guards():
    ...
    with pytest.raises(ValueError):
        _csk(nib_d=K_CSK)                    # nose corner (6+7) would reach past the rim
    with pytest.raises(ValueError):
        _csk(nib_l=12.3)                     # nib_l is not a free parameter for DIN 604
    with pytest.raises(ValueError):
        _cup(nib_l=None)                     # the cup form requires nib_l
    with pytest.raises(ValueError):
