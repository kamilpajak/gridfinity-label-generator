<script lang="ts">
	...
	function handleAddCurrentLabel() {
		if (!canAddLabel || !isFormValid || !hasContent) return;
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						>
							<ToggleGroupItem
								value="fastener"
								class="min-h-[34px] w-full flex-1 rounded-md px-2 text-xs font-semibold text-slate-500 hover:bg-transparent hover:text-slate-300 data-[state=on]:bg-slate-800 data-[state=on]:text-white data-[state=on]:shadow-sm data-[state=on]:ring-1 data-[state=on]:ring-slate-700"
			...
			{:else}
				...
				<div class="space-y-5">
					...
					<Button
						...
						disabled={!canAddLabel || !isFormValid || !hasContent}
