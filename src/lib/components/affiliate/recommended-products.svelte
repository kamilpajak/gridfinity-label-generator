...
>
	...
	<div class="space-y-0 p-5">
		{#each affiliateProducts as product, index (product.id)}
			{@const IconComponent = getProductIcon(product.id)}
			{@const isLast = index === affiliateProducts.length - 1}
				...
				<div class="flex gap-3">
					...
					<div class="min-w-0 flex-1">
						...
						<div class="flex items-center justify-between">
							...
							{#if product.badge}
								<span
									class="rounded bg-emerald-100 px-2 py-0.5 text-[10px] font-medium text-emerald-700"
								>
									{product.badge}
								</span>
