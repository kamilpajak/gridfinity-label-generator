...
DATA = Path("catalog/dimensions/turnbuckle_nuts.json")
_FORBIDDEN = ("reyher", "stalmut")

_BASES = ("din1479",)
_ALIASES = {"din1479d": "din1479"}
_DIN_CODE = {"din1479": "1479", "din1479d": "1479"}
...
def test_family_and_hardware_type():
    ...
    for sid, entry in entries.items():
        if "alias_of" in entry:
            continue
        assert entry["family"] == "turnbuckle_nut", f"{sid}: unexpected family {entry['family']}"
...
def test_aliases_resolve_to_a_real_base_without_chaining():
    ...
    for alias_id, base_id in _ALIASES.items():
        assert alias_id in entries, f"{alias_id} missing from turnbuckle_nuts.json"
        ...
        assert entries[alias_id]["hardwareType"] == "nut"
